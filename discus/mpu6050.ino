...
void mpu_get_event()
{
  ...
  float timeR =( float(millis()) - float(time_start_t)) / 10000.0F;
  NVS_Save_DATA(timeR ,g.gyro.x,g.gyro.y,g.gyro.z,a.acceleration.x, a.acceleration.y,a.acceleration.z);
...
void NVS_Save_DATA(float time_t , float Gyroscope_X,float Gyroscope_Y,float Gyroscope_Z, float Accelerometer_X, float Accelerometer_Y, float Accelerometer_Z)
{
	...
	temp = String(time_t) + " ";
	temp += String(Gyroscope_X) + " ";
  temp += String(Gyroscope_Y) + " ";
  temp += String(Gyroscope_Z) + " ";
  temp += String(Accelerometer_X) + " ";
  temp += String(Accelerometer_Y) + " ";
  temp += String(Accelerometer_X) + "\n";
